...
const fixture = (name) => new URL(`./fixtures/short-chain-baseline/${name}`, import.meta.url);
...
test('fixed-project regression keeps exact routing, project identity, prompt budget, and compile latency', async () => {
...
test('Short-Chain implementation has no analysis-report or execution-document dependency', async () => {
  // The short-chain runtime source files are read directly for the
  ...
  const sources = await Promise.all([
    readFile(new URL('./short-chain/compile.js', `file:///${pkgRoot.replace(/\\/g, '/')}`), 'utf8'),
    readFile(new URL('./short-chain/prompt-compiler.js', `file:///${pkgRoot.replace(/\\/g, '/')}`), 'utf8'),
    readFile(new URL('./short-chain/template-router.js', `file:///${pkgRoot.replace(/\\/g, '/')}`), 'utf8'),
    readFile(new URL('../../apps/desktop/src/main/image-generation/short-chain-service.ts', import.meta.url), 'utf8'),
  ]);
