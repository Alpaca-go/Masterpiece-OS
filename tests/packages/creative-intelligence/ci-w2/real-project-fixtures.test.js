...
// ("ä¹å·žç¾Žå­¦" / "ä¸€å‰‚è‰¯æ–?). They are intentionally NOT named after
// any specific brand, but their visual/structural shape â€?Direction
// family, locked assets, prohibited mutations â€?is identical to what
...
//
// The full real-project end-to-end retest ("ä¹å·žç¾Žå­¦" / "ä¸€å‰‚è‰¯æ–?)
...
// for the retest report (Part Q Â§18).
//
// Spec mapping (Part Q Â§18):
//   Q01 generic B2B brand â†?contract ready, 3 candidates, no auto-approval
//   Q02 generic B2C brand â†?contract ready, 3 candidates, no auto-approval
//   Q03 explicit user approval â†?ApprovedVisualAnchor exists, history
//   Q04 re-approve with different candidate â†?approvalRevision + 1
//   Q05 retry â†?does NOT replace existing approval
//   Q06 selection revision change â†?old approval invalidated
//   Q07 canon version change â†?old approval invalidated
...
// Shape: real-project (a) â€?B2B service platform (analog of ä¹å·žç¾Žå­¦)
// "é«˜ç«¯åŒ»ç¾Žå…¨é“¾ç”Ÿæ€å¹³å? â†?æ——èˆ°å“ç‰Œç©ºé—´æ•ˆæžœå›?â†?material-led, calm tones
const B2B_SHAPE = {
  directionFamily: 'material-led',
...
// Shape: real-project (b) â€?B2C packaged product (analog of ä¸€å‰‚è‰¯æ–?
// ä¸­å¼æ»‹è¡¥ / é£Ÿç–—å“ç‰Œ â†?åŒ…è£…è§†è§‰å‡çº§ â†?food-ingredient-led
const B2C_SHAPE = {
...
// ---------------------------------------------------------------------------
// Q01: B2B platform project â†?contract ready, 3 candidates, no auto-approval
// ---------------------------------------------------------------------------

test('Q01: B2B platform shape (analog of ä¹å·žç¾Žå­¦) â†?contract ready, 3 candidates, no auto-approval', async () => {
...
// ---------------------------------------------------------------------------
// Q02: B2C packaged product project â†?contract ready, 3 candidates, no auto-approval
// ---------------------------------------------------------------------------

test('Q02: B2C packaged product shape (analog of ä¸€å‰‚è‰¯æ–? â†?contract ready, 3 candidates, no auto-approval', async () => {
  const dataDir = await newTmpDir();
  try {
    const compileResult = buildAnchorProductionContract({
      projectId: 'p-real',
...
// ---------------------------------------------------------------------------
// Q03: explicit user approval â†?ApprovedVisualAnchor exists, history preserved
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Q04: re-approve with different candidate â†?approvalRevision + 1, history preserved
// ---------------------------------------------------------------------------
...
test('Q04: re-approve with different candidate advances approvalRevision and preserves history', async () => {
  ...
  try {
    ...
    const second = await service.approveAnchorCandidate('run-q04', ws.candidates[2].id, 'Second pick â€?better ingredient close-up');
...
// ---------------------------------------------------------------------------
// Q06: parent selection revision change â†?old approval invalidated
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Q08: no auto-selection â€?generated candidate is never pre-marked "approve"
// ---------------------------------------------------------------------------

test('Q08: generated candidates are not pre-marked approveable â€?verdict gates are user-decided', async () => {
...
test('Q10: 3 candidates Ã— evaluations â€?one evaluation record per candidate with structured verdicts', async () => {
  const dataDir = await newTmpDir();
  try {
    const service = makeService(dataDir);
