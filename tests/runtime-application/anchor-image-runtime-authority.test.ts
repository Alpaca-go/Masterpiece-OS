// Anchor image runtime authority regression suite.
//
...
// ---------------------------------------------------------------------------
// C: CI_ANCHOR_IMAGE_PROFILE_REQUIRED �?fail-closed when apiProfileId
...
test('CI-W1C.1 C (dynamic): runtime-services submitAnchorGeneration fails closed when apiProfileId is missing', async () => {
  ...
  // inside submitAnchorGeneration. This is the regression lock �?the
...
// ---------------------------------------------------------------------------
// M01: Seedream profile + supported 16:9 �?PASS (gate not blocked).
...
test('CI-W1C.1 M04: old wrong override (modelId: input.modelId) would have broken �?confirms pre-fix call shape is no longer present', async () => {
  ...
  assert.equal(profileRun.run.modelId, SEEDREAM_MODEL, 'M04: no override �?profile model wins');
