...
test('report schema requires the five action decisions and forbids v4 three-state decisions', async () => {
  const schema = await fs.readFile(path.resolve('prompts', 'analysis', 'report-schema.md'), 'utf8');
...
test('Execution Core template remains concise and contains all execution fields', async () => {
  const core = await fs.readFile(path.resolve('prompts', 'analysis', 'execution-core-template.md'), 'utf8');
  for (const field of [
