...
/**
 * §3 通用「闭合/徽章型专属图形」形态词表：只描述禁止复制的图形*形态*，与任何具体项目无关。
 * 具体项目的专属图形词必须来自数据（referenceIdentityBoundary.forbiddenSignatureGraphics），不得硬编码。
 */
export const CLOSED_BADGE_FORM_PATTERN =
  /徽章|徽标|印章|图章|完整椭圆|完整圆形|正圆(?:徽标|图形)|闭合(?:边框|外框|轮廓|图形|环)|环形边框|封闭外框|封闭纹样环|专属装饰带/u;
...
/**
 * §3 依据参考身份边界数据构建「禁止复制的参考专属图形」词匹配器；无有效词时返回 null。
 * 保持项目无关：词表完全来自运行期数据（如 forbiddenSignatureGraphics / 参考身份术语）。
 */
export function buildForbiddenSignatureGraphicPattern(terms: readonly string[]): RegExp | null {
  const tokens = unique(
    terms
      .flatMap((term) => term.split(/[、,，;；/｜|]+/u))
      .map((term) => term.trim())
      .filter((term) => term.length >= 2)
  );
  return tokens.length ? new RegExp(tokens.map(escapeRegExp).join('|'), 'u') : null;
}

/** §3 判断一段文本是否描述了禁止复制的参考专属 / 闭合徽章型图形。 */
export function describesSignatureGraphic(text: string, forbiddenPattern: RegExp | null): boolean {
  if (!text) return false;
  if (CLOSED_BADGE_FORM_PATTERN.test(text)) return true;
  return Boolean(forbiddenPattern && forbiddenPattern.test(text));
}

/** §3 判断某条参考风格规则是否属于禁止复制的专属图形：以继承层级(surface)与数据词表为准。 */
function isSignatureGraphicRule(rule: ReferenceStyleRule, forbiddenPattern: RegExp | null): boolean {
  if (rule.inheritanceLevel === 'surface') return true;
  return describesSignatureGraphic(rule.rule, forbiddenPattern);
}
...
function buildGenerationTaskDefinitionSection(
  current: CurrentProjectProfile,
  strategy: Omit<ReferenceFirstStrategy, 'betaClosure'>,
  primaryCarriers: StyleCarrier[],
  rule: TouchpointVisualRule | undefined,
  task: ReferenceFirstGenerationContext | undefined,
  forbidden: string[],
  outputType: GenerationOutputType
): string {
  const subject = rule?.primarySubjectType ?? 'graphic_system';
  const photographyRule = rule?.productPhotographyAllowed
    ? (rule.productPhotographyMayDominate ? '允许产品摄影，且可作为画面主导' : '允许产品摄影，但不得主导画面')
    : '不使用产品摄影，以系统化图形与版式为主';
  const requiredObjects = unique([
    `当前项目品牌标识：${current.brandName}`,
    ...primaryCarriers.map((item) => `Primary Style Carrier：${item.description}`)
  ]);
  const optionalObjects = unique([
    ...(rule?.productPhotographyAllowed ? current.coreProducts.map((item) => `产品事实：${item}`) : []),
    ...strategy.projectGraphicAnchor.sourceElements.map((item) => `当前项目来源元素：${item}`)
  ]);
  // §11 Generation Task Definition：10 项字段全部来自运行期数据，保持项目无关。
  return `## 6. 当前任务执行规则与生成任务定义（Generation Task Definition）
- 输出类型：${outputType}
- 视觉主体：${subject}
...
### 6.1 生成任务定义
1. 当前任务职责：${task?.prompt?.split('\n')[0] ?? `围绕 ${outputType} 输出参考主导的视觉系统`}
2. 画面主体类型：${subject}
3. 必需对象：
${list(requiredObjects)}
4. 可选对象：
${list(optionalObjects, '无')}
5. 构图规则：${strategy.systemAnchor.layoutGrammar}
6. 字体规则：${strategy.systemAnchor.typographyHierarchy}
7. 材质规则：${strategy.systemAnchor.materialLanguage}
8. 摄影规则：${photographyRule}
9. Logo 使用规则：仅使用生图身份包中的当前项目 Logo 与字标，不得复制或近似参考品牌 Logo。
10. 禁止输出方向：
${list(forbidden)}`;
}
...
function compileGenerationBrief(
  ...
  taskReferenceArtifactPath?: string
) {
  ...
  const boundary = strategy.referenceIdentityBoundary;
  const forbidden = unique([
    ...boundary.forbiddenSignatureGraphics,
    ...boundary.forbiddenBrandNames.map((name) => `参考品牌名称：${name}`),
    ...boundary.forbiddenLogos,
    ...boundary.forbiddenProductNames,
    ...boundary.forbiddenCopy,
    '参考专属 / 闭合徽章型图形（仅可抽象继承其重复、密度与层级规律，不得复制形状）',
    '当前项目旧配色、旧版式、旧图形、旧摄影与旧空间表现'
  ]);
  const taskDefinitionSection = buildGenerationTaskDefinitionSection(
    current,
    strategy,
    primary,
    rule,
  ...
  const taskSubsetLine = taskReferenceArtifactPath
    ? `- 文件：${taskReferenceArtifactPath}`
    : '- 缺少真实参考子集：等待人工确认，禁止使用固定或推测路径。';
...
export function buildReferenceFirstBetaClosure(input: {
  ...
  const forbiddenSignaturePattern = buildForbiddenSignatureGraphicPattern(
  ...
  const signatureRules = input.referenceStyleProfile.graphicLanguage
    .filter((item) => isSignatureGraphicRule(item, forbiddenSignaturePattern));
  ...
  const base = {
    ...
    graphicReconstruction: {
      ...
      identitySimilarityRisk: describesSignatureGraphic(strategy.projectGraphicAnchor.reconstructedForm, forbiddenSignaturePattern)
  ...
  const briefOutputType: GenerationOutputType = 'anchor_vi_system';
  const briefTaskReferenceArtifactPath = (protocol?.taskReferenceSubsets ?? [])
    .find((item) => item.outputType === briefOutputType)?.artifactPath;
  const generationBriefMarkdown = compileGenerationBrief(
    current,
    strategy,
    base,
    briefOutputType,
    briefTaskReferenceArtifactPath
  );
  ...
  const projectGraphicAnchorIsNonBadge = !describesSignatureGraphic(strategy.projectGraphicAnchor.reconstructedForm, forbiddenSignaturePattern);
