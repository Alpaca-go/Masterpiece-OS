...
export interface BuildProjectVisualContextShortChainInput {
  ...
  previousContext?: ProjectVisualContextShortChain | null;
  generatedAt?: string;
}
...
function mergePromptSourceObject(
  ...
  const migrated = migrateProjectVisualContextShortChain(context).promptSourceObject;
 ...
 */
export function migrateProjectVisualContextShortChain(
  context: ProjectVisualContextShortChain,
 ...
 */
export function buildProjectVisualContextShortChain(
  input: BuildProjectVisualContextShortChainInput,
): ProjectVisualContextShortChain {
  const { project } = input;
  ...
  const confirmedDecisions: ProjectVisualContextShortChain['confirmedDecisions'] = Array.isArray(structured.confirmedDecisions)
  ...
  const migrated = migrateProjectVisualContextShortChain(context);
  return {
    ...migrated,
...
export function validateProjectVisualContextShortChain(
  value: unknown,
...
export async function writeProjectVisualContextShortChain(
  filePath: string,
  context: ProjectVisualContextShortChain,
): Promise<void> {
  const validation = validateProjectVisualContextShortChain(context);
  if (!validation.valid) {
    throw Object.assign(new Error(`Project Visual Context Short-Chain invalid: ${validation.errors.join('; ')}`), {
      code: 'PROJECT_VISUAL_CONTEXT_SHORT_CHAIN_INVALID',
    });
  }
  const result = await atomicWriteJsonWithRetry(filePath, context);
  if (!result.success) {
    throw Object.assign(new Error(result.errorMessage ?? 'Project Visual Context Short-Chain write failed'), {
      code: 'PROJECT_VISUAL_CONTEXT_SHORT_CHAIN_WRITE_FAILED',
    });
  }
}
