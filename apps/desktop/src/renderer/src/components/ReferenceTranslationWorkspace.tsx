...
import { readinessStatusUserNotice } from '../../../shared/readiness-messages';
...
type ReadinessStatus = GenerationReadinessGate['status'];

const READINESS_STATUS_LABELS: Record<ReadinessStatus, string> = {
  ready: '可进入生图',
  needs_review: '需人工确认后进入生图',
  blocked: '当前无法进入生图'
};

interface ReadinessReasonDescriptor {
  field: string;
  artifact: string;
  autoFixable: boolean;
...
// §18 阻断原因 → 失败字段 / 对应产物 / 是否可自动修复 / 是否需要人工确认。
const READINESS_REASON_DETAILS: Record<string, ReadinessReasonDescriptor> = {
  GENERATION_IDENTITY_PACK_EMPTY: {
    field: '生图身份包（identityPack）', artifact: 'current-project/generation-identity-pack.json',
    autoFixable: false, humanConfirmation: true,
    hint: '未确认结构进入生图身份包：缺少可用的当前项目身份资产。'
  },
  GENERATION_IDENTITY_PACK_GRANULARITY_INVALID: {
    field: '生图身份包粒度', artifact: 'current-project/generation-identity-pack.json',
    autoFixable: false, humanConfirmation: true,
    hint: 'Locked Asset 中混入了旧版式/旧风格，需按颗粒度拆分后再确认。'
  },
  STRUCTURE_STATUS_UNRESOLVED: {
    field: '结构状态（structurePolicy.status）', artifact: 'current-project/structure-policy.json',
    autoFixable: false, humanConfirmation: true,
    hint: '未确认结构：产品/包装结构状态尚未由人工判定。'
  },
  REFERENCE_SIGNATURE_GRAPHIC_LEAK: {
    field: '参考专属图形隔离', artifact: 'reference/reference-signature-graphics.json',
    autoFixable: true, humanConfirmation: false,
    hint: 'Style Carrier 包含禁止复制的参考专属图形，可自动重构为非闭合、可延展的图形系统。'
  },
  REFERENCE_SIGNATURE_GRAPHIC_IN_STYLE_CARRIERS: {
    field: 'Style Carrier 图形合规', artifact: 'reference/global-style-carrier-ranking.json',
    autoFixable: true, humanConfirmation: false,
    hint: 'Style Carrier 内检出参考专属/闭合徽章型图形，需移除或抽象化。'
  },
  ANCHOR_SINGLE_SOURCE_VIOLATION: {
    field: '新旧 Anchor 规则一致性', artifact: 'anchors/system-anchor.json',
    autoFixable: false, humanConfirmation: true,
    hint: '新旧 Anchor 规则冲突：系统锚点与项目图形锚点存在矛盾。'
  },
  REQUESTED_TASK_SUBSET_MISSING: {
    field: '请求任务参考子集', artifact: 'tasks/task-reference-subset.json',
    autoFixable: false, humanConfirmation: true,
    hint: '请求任务缺少真实参考子集：等待人工确认，禁止使用固定或推测路径。'
  },
  TASK_STYLE_CARRIER_INCOMPATIBLE: {
    field: '任务级 Style Carrier 数量', artifact: 'reference/global-style-carrier-ranking.json',
    autoFixable: false, humanConfirmation: false,
    hint: '任务级主 Style Carrier 数量不在 3–6 的允许区间。'
  },
  GENERATION_TASK_DEFINITION_INCOMPLETE: {
    field: '生成任务定义（Generation Task Definition）', artifact: 'generation/generation-brief.md',
    autoFixable: true, humanConfirmation: false,
    hint: '生成任务定义缺少输出类型或主体类型，可根据任务子集自动补全。'
  },
  AUDIT_BRIEF_TASK_MISMATCH: {
    field: '执行文档与审计报告一致性', artifact: 'validation/cross-artifact-consistency.json',
    autoFixable: false, humanConfirmation: true,
    hint: '执行文档与审计报告上下文不一致：任务定义在两处存在冲突。'
  },
  STYLE_CARRIER_PRIORITY_INVALID: {
    field: 'Style Carrier 优先级', artifact: 'reference/global-style-carrier-ranking.json',
    autoFixable: false, humanConfirmation: false,
    hint: '缺少有效的 primary Style Carrier 或优先级标注非法。'
  },
  STYLE_CARRIER_PLACEHOLDER_LEAK: {
    field: 'Style Carrier 占位符', artifact: 'reference/global-style-carrier-ranking.json',
    autoFixable: true, humanConfirmation: false,
    hint: 'Style Carrier 中残留占位符文本，需替换为真实可读规则。'
  },
  TASK_REFERENCE_MATCH_CONTRADICTION: {
    field: '任务参考匹配', artifact: 'tasks/task-reference-subset.json',
    autoFixable: false, humanConfirmation: true,
    hint: '任务参考匹配矛盾：子集匹配度不足或未选中任何真实素材。'
  },
  UNVERIFIED_ASSET_ENTERED_GENERATION_PACK: {
    field: '未核验资产进入身份包', artifact: 'current-project/generation-identity-pack.json',
    autoFixable: false, humanConfirmation: true,
    hint: '未经核验的资产被放入生图身份包，需人工核验真实性。'
  },
  UNVERIFIED_ASSET_USED_AS_STRUCTURE_EVIDENCE: {
    field: '未核验资产作为结构证据', artifact: 'current-project/structure-policy.json',
    autoFixable: false, humanConfirmation: true,
    hint: '未经核验的资产被当作结构证据，需人工核验后再采用。'
  },
  GENERATION_BRIEF_MISSING_TASK_DETAILS: {
    field: '执行文档任务细节', artifact: 'generation/generation-brief.md',
    autoFixable: true, humanConfirmation: false,
    hint: '执行文档缺少目标任务的输出类型与任务细节，可自动补全。'
  }
};

function describeReadinessReason(raw: string): {
  code: string;
  detail: string;
  descriptor: ReadinessReasonDescriptor;
} {
  const [rawCode, ...rest] = raw.split(':');
  const code = rawCode || raw;
  const descriptor = READINESS_REASON_DETAILS[code] || {
    field: '未归类校验项',
    artifact: 'validation/generation-readiness.json',
    autoFixable: false,
    humanConfirmation: true,
    hint: '检出未归类的阻断项，需人工排查对应产物。'
  };
  return { code, detail: rest.join(':'), descriptor };
}
...
export function ReferenceTranslationWorkspace({ initialRunId = '', onBack }: Props) {
  ...
  async function start() {
    ...
    try {
      ...
      // §12 blocked 状态不得显示「可交给 GPT 生图」的成功提示；按实际准入状态呈现文案。
      const recon = result.reconstruction;
      const readinessStatus =
        recon?.generationReadinessResult?.status
        ?? recon?.generationReadiness?.status
        ?? 'ready';
      setNotice(readinessStatusUserNotice(readinessStatus));
    } catch (reason) {
      setError(cleanError(reason));
    } finally {
      setBusy(false);
  ...
  if (selectedRun && reconstruction) {
    const readiness = reconstruction.generationReadiness;
    // §13 UI 消费单一事实源：阻断原因以 GenerationReadinessResult.rootIssues 为准。
    const readinessResult = reconstruction.generationReadinessResult;
    // §6 客户端阻断：Readiness 为 blocked 时，禁止导出可执行文档 / 进入生图入口。
    const generationBlocked =
      readiness?.status === 'blocked' || selectedRun.readinessStatus === 'blocked';
    ...
    return <div className="page report-page reference-translation-report">
      ...
      <div className="result-summary">
        ...
        <div><small>生图准入</small><strong className={`readiness-tag ${readiness?.status || selectedRun.readinessStatus || 'unknown'}`}>
          {readiness ? READINESS_STATUS_LABELS[readiness.status]
            : selectedRun.readinessStatus ? READINESS_STATUS_LABELS[selectedRun.readinessStatus]
            : '—'}
        </strong></div>
      </div>
      <div className="result-actions">
        <button className="button secondary" disabled={generationBlocked}
          title={generationBlocked ? '生图准入被阻断：请先修复下方阻断项' : undefined}
          onClick={() => { if (generationBlocked) return; void window.masterpiece.referenceTranslation.readReport(selectedRun.id)
      ...
      {generationBlocked && <div className="notice warn">生图准入被阻断（blocked）：当前不会生成可执行 Prompt，请先修复下方阻断项。</div>}
      {notice && <div className="notice ok">{notice}</div>}

      {readiness && <section className={`panel reference-readiness reference-readiness-${readiness.status}`}>
        <div className="section-heading">
          <span>{readiness.status === 'ready' ? '✓' : '!'}</span>
          <div>
            <h2>{READINESS_STATUS_LABELS[readiness.status]}</h2>
            <p>Generation Readiness Gate · {readinessStatusUserNotice(readiness.status)}</p>
          </div>
        </div>
        {readinessResult && readinessResult.rootIssues.length > 0 && <div className="reference-readiness-list">
          {readinessResult.rootIssues.map((issue) => (
            <div className="reference-readiness-item" key={issue.code}>
              <div className="reference-readiness-head">
                <code>{issue.code}</code>
                <div className="reference-readiness-flags">
                  <span className={issue.autoRepairable ? 'flag ok' : 'flag no'}>
                    {issue.autoRepairable ? '可自动修复' : '需手动修复'}
                  </span>
                  <span className={issue.requiresHumanReview ? 'flag warn' : 'flag ok'}>
                    {issue.requiresHumanReview ? '需人工确认' : '无需人工确认'}
                  </span>
                </div>
              </div>
              <p className="reference-readiness-hint">{issue.repairInstruction || issue.message}</p>
              <div className="reference-readiness-meta">
                <span><small>失败字段</small>{issue.path || '—'}</span>
                <span><small>对应产物</small><code>{issue.artifactPath || '—'}</code></span>
                {issue.receivedValue != null && (
                  <span><small>失败值</small>{String(issue.receivedValue)}</span>
                )}
                <span><small>来源校验器</small>{issue.sourceValidator}</span>
              </div>
            </div>
          ))}
        </div>}
        {!readinessResult && readiness.blockingReasons.length > 0 && <div className="reference-readiness-list">
          {readiness.blockingReasons.map((raw) => {
            const { code, detail, descriptor } = describeReadinessReason(raw);
            return <div className="reference-readiness-item" key={raw}>
              <div className="reference-readiness-head">
                <code>{code}</code>
                <div className="reference-readiness-flags">
                  <span className={descriptor.autoFixable ? 'flag ok' : 'flag no'}>
                    {descriptor.autoFixable ? '可自动修复' : '需手动修复'}
                  </span>
                  <span className={descriptor.humanConfirmation ? 'flag warn' : 'flag ok'}>
                    {descriptor.humanConfirmation ? '需人工确认' : '无需人工确认'}
                  </span>
                </div>
              </div>
              <p className="reference-readiness-hint">{descriptor.hint}</p>
              <div className="reference-readiness-meta">
                <span><small>失败字段</small>{descriptor.field}</span>
                <span><small>对应产物</small><code>{descriptor.artifact}</code></span>
                {detail && <span><small>命中对象</small>{detail}</span>}
              </div>
            </div>;
          })}
        </div>}
        {readinessResult && readinessResult.rootIssues.length === 0 && readinessResult.derivedIssues.length > 0 && <p className="reference-readiness-note">
          存在 {readinessResult.derivedIssues.length} 个下游衍生阻断（由上游根因引发，修复根因后自动解除），当前无独立根因阻断项。
        </p>}
        {readiness.status === 'needs_review' && readiness.blockingReasons.length === 0 && (!readinessResult || readinessResult.rootIssues.length === 0) && <p className="reference-readiness-note">
          结构状态、任务子集或资产真实性存在待人工复核项，请在确认后再进入生图。
        </p>}
        {readiness.warnings && readiness.warnings.length > 0 && <div className="reference-readiness-warnings">
          {readiness.warnings.map((warn) => <span key={warn}>非阻断提示：{warn}</span>)}
        </div>}
      </section>}
      ...
      <aside className="panel visual-translation-history">
        ...
        {runs.length ? <div className="visual-run-list">{runs.map((run) => (
          <div key={run.id} className={`visual-run-card ${run.status}`}>
            ...
            {run.readinessStatus && <small className={`readiness-tag ${run.readinessStatus}`}>生图准入：{READINESS_STATUS_LABELS[run.readinessStatus]}</small>}
