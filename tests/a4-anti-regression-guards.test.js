// A4 G-A4-01..G-A4-10 �?offline contract tests for the 6 new A4 guards
//
// These tests verify that:
//   1. Each A4 guard script can be spawned and exits 0 (PASS).
...
const GUARDS = [
  { id: 'a4-default-authority', script: 'scripts/verify-a4-default-authority.mjs' },
  { id: 'a4-frozen-prompt', script: 'scripts/verify-a4-frozen-prompt.mjs' },
  { id: 'a4-version-namespace', script: 'scripts/verify-a4-version-namespace.mjs' },
  { id: 'a4-legacy-desktop', script: 'scripts/verify-a4-legacy-desktop.mjs' },
  { id: 'a4-golden-mutation', script: 'scripts/verify-a4-golden-mutation.mjs' },
  { id: 'a4-secret-safety', script: 'scripts/verify-a4-secret-safety.mjs' },
];
...
for (const { id, script } of GUARDS) {
  test(`A4 guard ${id} PASSES against CURRENT repository`, () => {
...
test('A4 G-A4-01 default-authority guard catches an injected hardcoded default', () => {
  ...
  const guard = path.join(repoRoot, 'scripts/verify-a4-default-authority.mjs');
...
test('A4 G-A4-03 frozen-prompt guard records the canonical digests', () => {
  const guard = path.join(repoRoot, 'scripts/verify-a4-frozen-prompt.mjs');
  const content = readFileSync(guard, 'utf8');
...
test('A4 G-A4-05 version-namespace guard lists all the A4-forbidden patterns', () => {
  const guard = path.join(repoRoot, 'scripts/verify-a4-version-namespace.mjs');
  const content = readFileSync(guard, 'utf8');
...
test('A4 G-A4-06 legacy-desktop guard checks TRACKED dirs (not on-disk untracked)', () => {
  const guard = path.join(repoRoot, 'scripts/verify-a4-legacy-desktop.mjs');
  const content = readFileSync(guard, 'utf8');
...
test('A4 G-A4-07 golden-mutation guard records the canonical Golden digests', () => {
  const guard = path.join(repoRoot, 'scripts/verify-a4-golden-mutation.mjs');
  const content = readFileSync(guard, 'utf8');
...
test('A4 G-A4-10 secret-safety guard masks the matched value in its output', () => {
  const guard = path.join(repoRoot, 'scripts/verify-a4-secret-safety.mjs');
  const content = readFileSync(guard, 'utf8');
