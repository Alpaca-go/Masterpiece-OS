...
import {
  scanVNextEvidence,
  runVNextEvidenceCheckpoint,
} from '@masterpiece/runtime-core/application/image-generation/vnext-evidence-scanner.ts';
...
test('F-4: scanner reads 9 evidence files and extracts bindings (standard basis)', async () => {
  ...
  const bundle = await scanVNextEvidence({
...
test('F-4: scanner computes sha256 of output.png (NOT the one in run.json)', async () => {
  ...
  const result = await runVNextEvidenceCheckpoint({
    projectId: ctx.projectId,
    taskId: ctx.taskId,
    runId: ctx.runId,
    compileArtifactDir: ctx.compileArtifactDir,
...
test('F-4: runVNextEvidenceCheckpoint happy path produces a passing checkpoint', async () => {
  const ctx = await setupHappyProject({
...
test('F-4: runVNextEvidenceCheckpoint blocks on missing required file (run.json)', async () => {
  const ctx = await setupHappyProject({
    basis: 'standard',
    dropFiles: ['run.json'],
  });
