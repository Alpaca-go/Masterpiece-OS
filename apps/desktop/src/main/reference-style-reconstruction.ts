...
export function finalizeReferenceStyleReconstruction(input: {
  ...
  // § 执行性校验（colorRulesAreFlexible 等）在 reference-first 流程中由后续
  // enforceReferenceFirstDirection / Generation Readiness Gate 接管，此处仅做软校验：
  // 失败不阻断流程，但会在最终质量校验（validateReferenceStyleReconstruction）中
  // 以 visualDirectionIsExecutable=false 记录。这与质量路径中
  // 「referenceFirstStrategy 存在时强制 visualDirectionIsExecutable=true」保持一致——
  // 最终质量报告不会因此标记失败。
  try { validateVisualDirectionExecutability(visualReconstructionDirection, input.currentProjectProfile); } catch {
    /* soft failure — handled by downstream enforcement layers */
  }
