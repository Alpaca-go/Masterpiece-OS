...
// 用途: bundle spatial-preset-validation.ts 之后用 electron 跑.
...
//      node scripts/spatial-preset-validation/run-spatial-preset-validation.mjs
...
const appRoot = path.join(outputRoot, 'spatial-preset-validation-app');
const bundlePath = path.join(outputRoot, 'spatial-preset-validation.mjs');

await fs.mkdir(appRoot, { recursive: true });
await build({
  entryPoints: [path.join(desktopRoot, 'scripts', 'spatial-preset-validation', 'spatial-preset-validation.ts')],
  bundle: true,
  platform: 'node',
  format: 'esm',
  packages: 'external',
...
await fs.writeFile(path.join(appRoot, 'package.json'), `${JSON.stringify({
  name: 'masterpiece-spatial-preset-validation',
  private: true,
  type: 'module',
  main: '../spatial-preset-validation.mjs',
}, null, 2)}\n`, 'utf8');
