...
function buildConceptPrompt(ctx, synthesis) {
  ...
  const userMessage = [
    ...
    '',
    '# ID ASSIGNMENT (deterministic; you may omit the id field on each candidate)',
    'When you do not provide an explicit `id` on a candidate, the runtime parser auto-assigns one in declaration order:',
    '  - candidates[0] -> id "concept-ma-0"; candidates[1] -> "concept-ma-1"; ...',
    'candidates[i].opportunityRefs MUST reference opportunity IDs (auto-assigned "opp-i0" / "opp-i1" / ...).',
    'candidates[i].insightRefs MUST reference insight IDs (auto-assigned "insight-i0" / "insight-i1" / ...).',
    'Do NOT put opportunity or insight statement text in opportunityRefs / insightRefs — only IDs.',
    '',
    '# OUTPUT JSON SCHEMA',
    'schemaVersion must be exactly "0.1".',
    'projectId must equal the projectId above.',
...
function buildDirectionPrompt(ctx, synthesis, conceptSet) {
  ...
  const userMessage = [
    ...
    'sourceMap.excludedAuthorities MUST list every authority excluded from positive creative source (typically: visualAsset.*, old_visual_style, old_VI, old_poster, old_packaging, old_spatial, style_reference, structure_reference, spatial_reference).',
    '',
    '# ID ASSIGNMENT (deterministic; you may omit the id field on each direction)',
    'When you do not provide an explicit `id` on a direction, the runtime parser auto-assigns one in declaration order:',
    '  - directions[0] -> id "direction-ma-0"; directions[1] -> "direction-ma-1"; ...',
    'directions[i].conceptRefs MUST reference concept IDs (auto-assigned "concept-ma-0" / "concept-ma-1" / ...).',
    'directions[i].opportunityRefs MUST reference opportunity IDs (auto-assigned "opp-i0" / "opp-i1" / ...).',
    'directions[i].insightRefs MUST reference insight IDs (auto-assigned "insight-i0" / "insight-i1" / ...).',
    'Do NOT put any statement text in any refs field — only IDs.',
    '',
