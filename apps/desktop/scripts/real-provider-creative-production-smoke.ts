...
function summary(value: unknown): void {
  process.stdout.write(`CREATIVE_PRODUCTION_SMOKE_RESULT ${JSON.stringify(value)}\n`);
}
...
async function main(): Promise<void> {
  ...
  const imageGeneration = createImageGenerationService({
    ...
    emitRunUpdated: (progress) => {
      process.stdout.write(`CREATIVE_PRODUCTION_SMOKE_PROGRESS ${JSON.stringify({
  ...
  if (directionResult.direction.status !== 'ready'
    ...
    throw new Error(
      `创意生产真实 Provider 验收未完成：direction=${directionResult.direction.status}, image=${anchorResult.run.status}`,
    );
  }
}
...
app.whenReady().then(async () => {
  try {
    ...
    process.stderr.write(`CREATIVE_PRODUCTION_SMOKE_ERROR ${JSON.stringify(safe)}\n`);
    app.exit(1);
  }
});
