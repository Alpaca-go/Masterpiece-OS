...
// attached to VNextValidatedGenerationResult.similarityAudit. Failures
...
import type { VNextSimilarityAuditService } from '@masterpiece/runtime-core/application/image-generation/vnext-similarity-audit-service.ts';
...
const happyAuditResult: VNextSimilarityAuditResult = {
  scores: {
...
interface SetupOptions {
  ...
  auditService?: VNextSimilarityAuditService;
}
...
async function compileOnce(
  service: ReturnType<typeof createVNextImageGenerationService>,
...
function makeHappyAuditService(): VNextSimilarityAuditService & { calls: Array<{ projectId: string; runId: string }> } {
  ...
  }) as VNextSimilarityAuditService['audit'];
  const svc = { audit } as VNextSimilarityAuditService & { calls: typeof calls };
  svc.calls = calls;
  return svc;
}

function makeThrowingAuditService(): VNextSimilarityAuditService {
  return {
    audit: (async () => {
