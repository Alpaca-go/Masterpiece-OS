import { getShortChainTemplate } from './template-registry.js';

export const SHORT_CHAIN_TEMPLATE_ROUTER_VERSION = '1.0.0';

export function routeShortChainTemplates(task, options = {}) {
  ...
  const templates = ids.map((id) => {
    const template = getShortChainTemplate(id);
    if (!template) throw new Error(`No Short-Chain template registered for ${id}`);
