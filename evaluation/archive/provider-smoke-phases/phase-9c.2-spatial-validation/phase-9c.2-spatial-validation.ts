...
//   - Logo / locked asset detection (跟 createFileContextLoader / short-chain-service 一致)
//   - Reference role mapping: logo → identity_reference, structure anchor → structure_reference
//   - short-chain-style snapshot + sourceMap (schemaVersion: 'space-runtime-1.0', pipelineMode: 'space-runtime')
...
async function runOneBrand(
  ...
  // short-chain-style snapshot + sourceMap + references with correct roles.
  // Per short-chain-service line 396-400:
  ...
  const promptVersion = assetContract.snapshot.taskContract.taskId;  // use short-chain-style taskId
  const imageRun = await imageGeneration.startCompiledCreativeTask({
    projectId,
    compiledPrompt: compiled.markdown,
    promptVersion,
    snapshot: assetContract.snapshot,  // short-chain-style with lockedAssetIds/lockedFacts
    sourceMap: assetContract.sourceMap,  // short-chain-style with pipelineMode: 'space-runtime'
    references,
