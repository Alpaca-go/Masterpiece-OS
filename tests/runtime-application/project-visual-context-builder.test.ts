...
test('vNext context is built from project facts and structured data without report markdown', () => {
  ...
  assert.deepEqual(validateProjectVisualContext(context), { valid: true, errors: [] });
});

test('vNext context version increments independently of a report filename', () => {
  const first = buildProjectVisualContext({ project: project() });
  const withoutReport = { ...project(), lastReportFilename: null };
  const second = buildProjectVisualContext({
...
test('legacy vNext context migrates to a report-independent PromptSourceObject', () => {
  const legacy = buildProjectVisualContext({
    project: project(),
    generatedAt: '2026-07-30T00:00:00.000Z',
  });
  const withoutPromptSource = { ...legacy, promptSourceObject: undefined };
  const migrated = migrateProjectVisualContext(withoutPromptSource);
...
test('validated Visual Decision Packet is persisted beside the compatibility Prompt Source', () => {
  const base = buildProjectVisualContext({ project: project() });
  const packet = {
    schemaVersion: '1.0',
    projectId: 'project-1',
    projectFacts: {},
...
test('persisted vNext context migrates a legacy Packet even when Prompt Source already exists', () => {
  ...
  const migrated = migrateProjectVisualContext(persistedLegacy);
