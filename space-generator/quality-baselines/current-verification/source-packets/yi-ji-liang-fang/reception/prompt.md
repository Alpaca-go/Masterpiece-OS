...
Task: 生成一剂良方中医体验馆的接待与挂号空间效果图。以视平线高度单一广角透视呈现到达、接待台与后方品牌墙的层次关系，品牌识别由项目资产抽象转译，Logo 后期合成。
Aspect ratio: 16:9.
...
**Spatial Strategy**:
- 模块化陈列墙：提取药柜的'格子'概念，用于产品展示、药材陈列或书籍收纳，形成空间背景的节奏感
- 圆弧隔断：提取Logo的圆弧线条，用于候诊区与诊疗区的软性分隔，营造'包容/私密'感
...
- 基于'圆弧/包围'形态逻辑（源自Logo）的半封闭候诊/阅读空间
- 提取Logo辅助图形的网格矩阵式药材展示墙
- 现代化转译的药柜系统（木质层板、黄铜把手、磨砂玻璃门）
- 以'良'字图标为核心的视觉焦点区（如接待台、导视牌）
...
**Building DNA**:
- Spatial organization: 基于'圆弧/包围'形态逻辑（源自Logo）的半封闭候诊/阅读空间；提取Logo辅助图形的网格矩阵式药材展示墙；现代化转译的药柜系统（木质层板、黄铜把手、磨砂玻璃门）；以'良'字图标为核心的视觉焦点区（如接待台、导视牌）；融合金属、玻璃、洁净白色与现代中医元素的诊疗室
- Functional network nodes: 基于'圆弧/包围'形态逻辑（源自Logo）的半封闭候诊/阅读空间；提取Logo辅助图形的网格矩阵式药材展示墙；现代化转译的药柜系统（木质层板、黄铜把手、磨砂玻璃门）；以'良'字图标为核心的视觉焦点区（如接待台、导视牌）；融合金属、玻璃、洁净白色与现代中医元素的诊疗室
- Program nodes: 接待/挂号；候诊/阅读；诊疗/把脉；取药/治疗；药材/产品展示
