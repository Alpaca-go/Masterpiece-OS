...
/**
 * §6 / §15 Task Reference Subset 的规范产物相对路径（每个输出类型独立目录）。
 * 这是 artifactPath 的唯一来源，禁止使用 `tasks/${outputType}.json` 这类扁平固定路径。
 */
export function taskReferenceSubsetArtifactPath(outputType: GenerationOutputType): string {
  return `tasks/${outputType}/task-reference-subset.json`;
}
...
export function selectTaskReferences(
  ...
  const subsets = [...new Set(outputTasks)].map((outputType) => {
    ...
    // §6 仅当存在真实子集（有选中资产且非 insufficient）时才写入 artifactPath，
    // 否则留空以便 validateRequestedTaskCoverage 对缺失的请求任务阻断。
    const hasRealSubset = matchLevel !== 'insufficient' && selected.length > 0;
    return {
      outputType,
      selectedAssetIds: selected.map((item) => item.assetId),
      artifactPath: hasRealSubset ? taskReferenceSubsetArtifactPath(outputType) : undefined,
