...
import { createNodeCredentialStore } from './node-credential-store.ts';
...
const runtimePaths = createNodeRuntimePaths();
const nodeCredentials = createNodeCredentialStore(runtimePaths.credentials);
...
function settingsPath(): string {
  return runtimePaths.settingsFile;
}
...
function defaults(): StoredSettings {
  return {
    ...
    defaultDataPath: runtimePaths.defaultDataPath,
    cacheEnabled: true,
    logLevel: 'info',
...
async function hasCredential(profileId: string): Promise<boolean> {
  return nodeCredentials.has(profileId);
}
...
async function publicSettings(settings: StoredSettings): Promise<PublicSettings> {
  ...
  return {
    profiles,
    modelRegistry: listRegisteredModels().map((model: any) => ({
      ...model,
...
async function decryptApiKey(profileId: string): Promise<string> {
  return nodeCredentials.read(profileId);
}
...
export async function deleteApiProfile(profileId: string): Promise<PublicSettings> {
  ...
  await nodeCredentials.remove(profileId);
