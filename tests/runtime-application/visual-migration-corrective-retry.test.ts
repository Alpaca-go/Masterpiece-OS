...
test('VM-6 correction plan maps failures deterministically without an LLM', () => {
  ...
  assert.deepEqual(plan.canonRulesUsed, ['keep own logo', 'own name', 'logo', 'warm palette', 'clear grid', 'line rhythm', 'paper texture', 'repeat rhythm', 'foreign brand', 'foreign logo', 'signature motif']);
...
test('VM-6 corrective retry preserves authority/reference set and persists child bindings before Provider', async (t) => {
  ...
  const sourceMap = JSON.parse((await f.store.readRunArtifact('child', 'prompt-source-map.json'))!.toString('utf8'));
  assert.deepEqual(sourceMap.visualMigrationCorrection.canonRulesUsed, result.plan.canonRulesUsed);
