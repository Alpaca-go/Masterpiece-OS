...
export function validateCrossArtifactConsistency(input: {
  ...
  auditTaskDefinitionPresent?: boolean;
  briefTaskDefinitionPresent?: boolean;
}): CrossArtifactConsistencyValidation {
  ...
  // §13：Generation Task Definition 必须同时存在于审计报告与执行文档。
  // 只有当调用方显式声明其中一侧缺失时才判为不一致；未声明视为一致（向后兼容）。
  const taskDefinitionMatches = !(
    input.auditTaskDefinitionPresent === false
    || input.briefTaskDefinitionPresent === false
  );
  if (!taskDefinitionMatches) contradictions.push('AUDIT_BRIEF_TASK_DEFINITION_MISMATCH');
