...
import {
  ...
  taskReferenceSubsetArtifactPath,
  type GenerationReadinessOrchestratorResult,
  type GenerationReadinessResult
...
/** 兼容旧运行目录的扁平命名（按输出类型），仅用于并行写出便于人工查阅。 */
const TASK_SUBSET_FILENAMES: Record<string, string> = {
...
const TASK_SUBSET_DIR = 'task-reference-subsets';

/**
 * §6 为缺失 artifactPath 的真实 Task Reference Subset 兜底填充规范产物路径。
 * 正常情况下 selectTaskReferences 已在筛选期写入；此处覆盖旧持久化协议缺字段的复用场景。
 * 保持 §6 语义：仅对有真实内容（非 insufficient 且有选中资产）的子集补路径，缺失子集仍留空以阻断。
 */
function enrichTaskReferenceSubsetArtifactPaths(protocol: AssetSelectionProtocolResult): void {
  for (const subset of protocol.taskReferenceSubsets) {
    if (subset.artifactPath) continue;
    const hasRealSubset = subset.matchLevel !== 'insufficient' && subset.selectedAssetIds.length > 0;
    if (hasRealSubset) subset.artifactPath = taskReferenceSubsetArtifactPath(subset.outputType);
  }
}
...
/**
 * §12/§14 落盘 Generation Readiness 闭环产物（规范相对路径，见 READINESS_ARTIFACT_PATHS）。
 * 与 beta 产物解耦：这里只写协议形态的闭环校验产物，供审计报告与执行文档共享同一 manifest。
 */
async function persistGenerationReadinessArtifacts(
  root: string,
  orchestration: GenerationReadinessOrchestratorResult,
  extras: {
    referenceSignatureGraphics: unknown;
    taskReferenceSubsets: unknown;
    systemAnchor: unknown;
    projectGraphicAnchor: unknown;
    generationBriefMarkdown: string;
    analysisAuditMarkdown: string;
  }
): Promise<void> {
  const paths = READINESS_ARTIFACT_PATHS;
  const dirs = new Set(
    Object.values(paths)
      .map((relative) => path.dirname(path.join(root, relative)))
      .filter((directory) => directory !== root)
  );
  await Promise.all([...dirs].map((directory) => fs.mkdir(directory, { recursive: true })));
  await Promise.all([
    writeJson(path.join(root, paths.generationReadiness), orchestration.generationReadiness),
    writeJson(path.join(root, paths.generationReadinessResult), orchestration.generationReadinessResult),
    writeJson(path.join(root, paths.crossArtifactConsistency), orchestration.crossArtifactConsistency),
    writeJson(path.join(root, paths.structurePolicy), orchestration.structurePolicy),
    writeJson(path.join(root, paths.generationIdentityPack), orchestration.identityPack),
    writeJson(path.join(root, paths.globalStyleCarrierRanking), orchestration.globalStyleCarrierRanking),
    writeJson(path.join(root, paths.referenceSignatureGraphics), extras.referenceSignatureGraphics),
    writeJson(path.join(root, paths.taskReferenceSubset), extras.taskReferenceSubsets),
    writeJson(path.join(root, paths.systemAnchor), extras.systemAnchor),
    writeJson(path.join(root, paths.projectGraphicAnchor), extras.projectGraphicAnchor),
    writeJson(path.join(root, paths.generationContextManifest), orchestration.generationContextManifest),
    // §7 落盘执行清单：证明每个已注册 Validator 都被执行（无静默跳过）。
    writeJson(path.join(root, paths.validatorExecutionManifest), orchestration.validatorExecutionManifest),
    // §15 落盘 Resolved Project Facts 单一来源。
    writeJson(path.join(root, paths.resolvedProjectFacts), orchestration.resolvedProjectFacts),
    fs.writeFile(path.join(root, paths.generationBrief), extras.generationBriefMarkdown, 'utf8'),
    fs.writeFile(path.join(root, paths.analysisAuditReport), extras.analysisAuditMarkdown, 'utf8')
  ]);
}

/** Brief 是否声明了生成任务定义（§11），用于 cross-artifact taskDefinition 一致性判定。 */
function briefStatesTaskDefinition(markdown: string): boolean {
  return /生成任务定义|Generation Task Definition/u.test(markdown || '');
}

/**
 * §14 生产链路 Generation Readiness 闭环入口（主流程与继续分析共用）。
 * 将生产真实产物映射为协议输入，运行编排器，落盘规范产物，并把 readiness 附加到 reconstruction。
 * 不抛错、不阻断报告生成；缺少运行时上下文时安全跳过并返回 null。
 */
interface GenerationReadinessClosureResult {
  readiness: GenerationReadinessGate;
  /** §ValidationIssue 聚合与 Readiness 单一事实源（Audit / Blocked / UI 共用）。 */
  result: GenerationReadinessResult;
  /** §6 blocked 时的阻断报告（不含可执行 Prompt）；非 blocked 时为 null。 */
  blockedReportMarkdown: string | null;
}

async function applyGenerationReadinessClosure(params: {
  runId: string;
  root: string;
  strategy: ReferenceFirstStrategy;
  protocol: AssetSelectionProtocolResult;
  reconstruction: ReferenceStyleReconstruction;
}): Promise<GenerationReadinessClosureResult | null> {
  const { runId, root, strategy, protocol, reconstruction } = params;
  const closure = strategy.betaClosure;
  const runtime = await readJson<ProjectRuntimeContext>(
    path.join(root, 'current-project-runtime-context.json')
  ).catch(() => null);
  if (!runtime) return null;

  const requestedTaskOutputTypes = protocol.requestedTasks?.length
    ? [...new Set(protocol.requestedTasks.map((task) => task.outputType))]
    : [...new Set(protocol.taskReferenceSubsets.map((subset) => subset.outputType))];
  const requiredTaskOutputTypes = protocol.requestedTasks?.length
    ? [...new Set(protocol.requestedTasks.filter((task) => task.required).map((task) => task.outputType))]
    : requestedTaskOutputTypes;

  const orchestration = orchestrateGenerationReadiness({
    jobId: runId,
    runtime,
    assetDecisions: protocol.currentProjectAssetDecisions,
    facts: strategy.evidenceBoundFacts || [],
    observedCopy: closure.observedCopy || [],
    styleCarriers: closure.styleCarrierRanking || [],
    referenceSignatureGraphics: closure.referenceSignatureGraphics || [],
    requestedTaskOutputTypes: requestedTaskOutputTypes as GenerationOutputType[],
    requiredTaskOutputTypes: requiredTaskOutputTypes as GenerationOutputType[],
    taskReferenceSubsets: protocol.taskReferenceSubsets,
    systemAnchor: strategy.systemAnchor,
    projectGraphicAnchor: strategy.projectGraphicAnchor,
    auditReport: closure.analysisAuditMarkdown,
    generationBrief: closure.generationBriefMarkdown,
    briefStatesTaskDefinition: briefStatesTaskDefinition(closure.generationBriefMarkdown),
    legacyObservations: (closure.legacyVisualObservations || []).map((fact) => fact.value),
    signatureGraphicLeak: protocol.signatureGraphicLeakValidation,
    identityAssetIds: closure.generationIdentityPack?.identityAssetIds || []
  });

  // §6 blocked 状态不得产出可执行 Prompt：改写落盘的 generation-brief 为阻断报告。
  const blocked = orchestration.generationReadiness.status === 'blocked';
  const blockedReportMarkdown = blocked
    ? compileBlockedGenerationReport({
        identityPack: orchestration.identityPack,
        readiness: orchestration.generationReadiness,
        readinessResult: orchestration.generationReadinessResult
      })
    : null;

  // §10 审计报告统一追加「生成准备状态」章节，读取单一事实源（不得重新计算）。
  const readinessStatusSection = compileReadinessStatusSection(
    orchestration.generationReadinessResult
  );
  const augmentedAuditMarkdown = `${closure.analysisAuditMarkdown}\n\n${readinessStatusSection}`;

  await persistGenerationReadinessArtifacts(root, orchestration, {
    referenceSignatureGraphics: closure.referenceSignatureGraphics || [],
    taskReferenceSubsets: protocol.taskReferenceSubsets,
    systemAnchor: strategy.systemAnchor,
    projectGraphicAnchor: strategy.projectGraphicAnchor,
    generationBriefMarkdown: blockedReportMarkdown ?? closure.generationBriefMarkdown,
    analysisAuditMarkdown: augmentedAuditMarkdown
  });

  reconstruction.generationReadiness = orchestration.generationReadiness;
  reconstruction.generationReadinessResult = orchestration.generationReadinessResult;
  return {
    readiness: orchestration.generationReadiness,
    result: orchestration.generationReadinessResult,
    blockedReportMarkdown
  };
}
...
export function createReferenceTranslationService(
  ...
  async function runUserInput(input: StartReferenceTranslationUserInput): Promise<ReferenceTranslationResult> {
    ...
    try {
      ...
      // §6 为每个 Task Reference Subset 填充真实产物路径，供执行文档与一致性校验读取。
      enrichTaskReferenceSubsetArtifactPaths(assetSelectionProtocol);
      await Promise.all([
      ...
      const taskSubsetDir = path.join(root, TASK_SUBSET_DIR);
      ...
      const namedTaskSubsetWrites = assetSelectionProtocol.taskReferenceSubsets.map((subset) => {
        const filename = TASK_SUBSET_FILENAMES[subset.outputType];
        if (!filename) {
          throw Object.assign(
      ...
      // §6 / §15 将有真实内容的子集写入规范产物路径（tasks/<output-type>/task-reference-subset.json），
      // 使 subset.artifactPath 指向的真实文件确实存在。
      const canonicalTaskSubsetWrites = assetSelectionProtocol.taskReferenceSubsets
        .filter((subset) => subset.artifactPath)
        .map(async (subset) => {
          const target = path.join(root, subset.artifactPath!);
          await fs.mkdir(path.dirname(target), { recursive: true });
          await writeJson(target, subset);
        });
      await Promise.all([
      ...
      const readinessGate = await applyGenerationReadinessClosure({
        runId,
        root,
        strategy: referenceFirst,
        protocol: assetSelectionProtocol,
      ...
      await Promise.all([
        ...
        fs.writeFile(path.join(root, reportFilename), readinessGate?.blockedReportMarkdown ?? finalized.markdown, 'utf8'),
      ...
      const completedRecord: ReferenceTranslationRunRecord = {
        ...
        readinessStatus: readinessGate?.readiness.status,
        error: null,
        lastError: null
      };
      ...
      return {
        run: completedRecord,
        reportMarkdown: readinessGate?.blockedReportMarkdown ?? finalized.markdown,
        reconstruction: finalized.reconstruction,
        assetSelectionProtocol
      };
    } catch (error) {
  ...
  async function resume(runId: string, requestedApiProfileId?: string): Promise<ReferenceTranslationResult> {
    ...
    // §6 复用流程同样按真实产物补齐 Task Reference Subset 路径（旧持久化可能缺失该字段）。
    enrichTaskReferenceSubsetArtifactPaths(assetSelectionProtocol);
