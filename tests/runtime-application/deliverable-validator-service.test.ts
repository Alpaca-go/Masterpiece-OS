...
import { createDeliverableValidatorService } from '@masterpiece/runtime-core/application/image-generation/deliverable-validator-service.ts';
...
test('vNext validator records unverified instead of trusting prompt metadata without vision evidence', async () => {
  ...
  const service = createDeliverableValidatorService(
    { paths: async () => ({
      root,
