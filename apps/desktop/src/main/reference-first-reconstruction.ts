...
import {
  buildForbiddenSignatureGraphicPattern,
  buildReferenceFirstBetaClosure,
  describesSignatureGraphic
} from './reference-first-beta-closure.ts';
...
export function buildReferenceFirstStrategy(input: {
  ...
  // §3 以数据（参考身份术语 + 参考排除术语）+ 通用闭合/徽章形态词判定专属图形，保持项目无关。
  const forbiddenSignaturePattern = buildForbiddenSignatureGraphicPattern(
    unique([...(input.referenceIdentityTerms ?? []), ...style.excludedIdentityTerms])
  );
  const safeGraphicForm = describesSignatureGraphic(rawGraphicForm, forbiddenSignaturePattern)
