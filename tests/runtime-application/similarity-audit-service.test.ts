...
// ShortChainSimilarityAuditResult. It is invoked by vnext-service.startValidated
...
//   - LLM returns non-integer scores → audit throws (assertShortChainSimilarityAudit
...
import {
  createSimilarityAuditService,
  type ShortChainSimilarityAuditReasoner,
} from '@masterpiece/runtime-core/application/image-generation/similarity-audit-service.ts';
...
function mockReasoner(opts: MockOptions = {}): ShortChainSimilarityAuditReasoner & {
  ...
  const wrapped = fn as ShortChainSimilarityAuditReasoner & { calls: typeof calls };
  wrapped.calls = calls;
  return wrapped;
}
...
async function makeService(opts: {
  root: string;
  reasoner?: ShortChainSimilarityAuditReasoner;
  ...
  return createSimilarityAuditService({
...
test('F-2: LLM returns non-integer scores → audit throws (assertShortChainSimilarityAudit rejects)', async () => {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), 'masterpiece-f2-'));
  const reasoner = mockReasoner({
    reportJson: {
