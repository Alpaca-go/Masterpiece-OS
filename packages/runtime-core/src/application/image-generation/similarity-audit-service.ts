...
//     attached to ShortChainValidatedGenerationResult.similarityAudit for
...
export interface ShortChainSimilarityAuditInput {
  projectId: string;
  runId: string;
 ...
 */
export type ShortChainSimilarityAuditReasoner = (input: {
  prompt: {
...
interface AuditDeps {
   ...
   */
  createReasoner?: (credentials: Credentials) => ShortChainSimilarityAuditReasoner;
...
function readScore(value: unknown, key: string): number {
  if (typeof value !== 'number' || !Number.isFinite(value) || !Number.isInteger(value)) {
    throw new Error(`ShortChainSimilarityAudit: ${key} must be an integer; got ${JSON.stringify(value)}`);
  }
  if (value < 1 || value > 5) {
    throw new Error(`ShortChainSimilarityAudit: ${key} must be in 1..5; got ${value}`);
  }
  return value;
}

function buildAuditPrompt(input: {
  targetScene: ShortChainSimilarityAuditInput['targetScene'];
  referenceSceneRelation: ShortChainReferenceSceneRelation;
...
function defaultCreateReasoner(credentials: Credentials): ShortChainSimilarityAuditReasoner {
  ...
  }) as ShortChainSimilarityAuditReasoner;
}

export function createSimilarityAuditService(deps: AuditDeps) {
  ...
  async function persist(
    projectId: string,
    runId: string,
    result: ShortChainSimilarityAuditResult,
  ): Promise<{ path: string }> {
  ...
  async function audit(input: ShortChainSimilarityAuditInput): Promise<ShortChainSimilarityAuditResult> {
    ...
    const pass = assertShortChainSimilarityAudit(scores);
    ...
    const result: ShortChainSimilarityAuditResult = {
      scores,
      pass,
      rationale,
      metadata: {
...
export type SimilarityAuditService = ReturnType<typeof createSimilarityAuditService>;
