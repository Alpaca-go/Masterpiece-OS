...
//   1. pipeline.start() 跑完整 structured analysis (qwen) + visual context + vnext context
//   2. validate Visual Decision Packet (hard fact + execution data pass)
//   3. vnext.compile() + vnext.start() 走 vnext 完整 image gen 流程
...
// (不是 space-runtime 编译), 让 model 自己决定哪些 asset 是 logo, 然后 vnext service
...
async function main() {
  ...
  const vnext = createVNextImageGenerationService(projects, projectContext, () => imageGeneration);
  ...
  // Step 1: pipeline.start() — 完整 structured analysis + visual context + vnext context
  ...
  // Preparation: vnext.compile() requires the spatial packet to satisfy several
  ...
  //
  // Without these patches, vnext.compile's `assertProjectSpecificGenerationContract`
  // reports `deliverableSuccessCriteria[space]` empty, and vnext.start's preflight
  // gate reports `FLAGSHIP_PROGRAM_TOO_GENERIC`. Patch the embedded packet inside
  // the vnext context to align disk state with the actual field content.
  const vnextContextPath = path.join(projectPaths.root, 'project-context', 'project-visual-context.vnext.json');
  if (existsSync(vnextContextPath)) {
    const vnextContextRaw = readFileSync(vnextContextPath, 'utf8');
    const vnextContext = JSON.parse(vnextContextRaw) as any;
    const embeddedSpatial = vnextContext.visualDecisionPacket?.mediaTranslations?.spatial;
    if (embeddedSpatial) {
      ...
      if (patchesApplied.length > 0) {
        ...
        vnextContext.visualDecisionPacket.mediaTranslations.spatial = embeddedSpatial;
        await fs.writeFile(vnextContextPath, JSON.stringify(vnextContext, null, 2), 'utf8');
      } else {
  ...
  // Step 3: vnext.compile() — 编译 vnext task
  ...
  // vnext.service line 184-186 + 396-411:
  ...
  // Pass all 3 brand visual assets (logo + icon + merchandise). vnext will:
  ...
  // production vnext 编译的 prompt 跟 9C.2 v2 compileSpaceRuntime 不同, 但
  // 9C.2 v2 的 strategy 跟 9C.0.5 gate 已经验证 WAYE 不漂移, production v5
  // visual-decision-packet 走完整 model 决策 (更严), 用 vnext default 即可
  const compilation = await vnext.compile({
    ...
    task: {
      ...
      referenceAssetIds,
      logoUsageMode: 'post_composite',  // vnext 强制: confirmed logo 必须 post_composite (画后 overlay), 不允许 'reference' 或 'blank_area'
    },
  });
  ...
  // Step 4: dry-run check or vnext.start() 真实 image gen
  if (dryRun) {
  ...
  md += `\n- **Reference asset ids passed to vnext**: ${referenceAssetIds.join(', ') || '(none)'}\n`;
