...
test('compiler reads Visual Decision Packet directly and covers all project blocks', () => {
  ...
  assert.equal(result.compiledPrompt.trace.compilerVersion, 'short-chain-prompt-compiler@4.3.0');
...
test('Jiuzhou automatic Prompt meets all 22 Golden backtrace atoms', () => {
  ...
  const audit = generateGoldenBacktraceAudit({
    items: [
      ...
      { id: 'JZ-17', goldenContent: '色彩关系：主色珍珠白、辅助暖灰、点缀低饱和矿物紫', matchGroups: [['珍珠白'], ['暖灰'], ['低饱和矿物紫']], contentType: 'media_translation' },
      ...
      { id: 'JZ-20', goldenContent: '专业、可信赖和长期价值', matchGroups: [['专业'], ['可信赖']], contentType: 'creative_decision' },
