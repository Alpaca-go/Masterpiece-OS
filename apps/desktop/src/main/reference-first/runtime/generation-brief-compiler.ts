...
import { compileBlockedGenerationReport } from './blocked-generation-report-compiler.ts';
...
export function compileGenerationBrief(input: {
  ...
  /** §6 Generation Readiness Gate 结果。status=blocked 时禁止输出可执行 Prompt。 */
  readiness?: GenerationReadinessGate;
}): string {
  // §6 blocked 状态不得生成可执行 Prompt：只输出阻断报告。
  if (input.readiness && input.readiness.status === 'blocked') {
    return compileBlockedGenerationReport({
      identityPack: input.identityPack,
