...
test('v5 performs one reasoning call, skips Compilers and publishes one official document', async () => {
  ...
  const execution = await runAnalysisPipeline(input, {
    projectRoot,
    output,
...
test('v5 reuses an exact prompt result without a second model call', async () => {
  ...
  await runAnalysisPipeline(input, { projectRoot, output, deepCreativeDirectorReasoner: reasoner });
  const second = await runAnalysisPipeline(input, { projectRoot, output, deepCreativeDirectorReasoner: reasoner });
  assert.equal(calls, 1);
...
test('v5 records failure stage, wall-clock time, and model-call start', async () => {
  const { projectRoot, input, output } = await fixture();
  await assert.rejects(runAnalysisPipeline(input, {
    projectRoot,
    output,
