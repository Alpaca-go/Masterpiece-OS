...
export function buildConceptIdeationPrompt(input: ConceptIdeationPromptInput): ConceptIdeationPromptOutput {
  ...
  const userMessage = [
    ...
    'candidates[i].opportunityRefs MUST reference opportunity IDs (either the ones the synthesis wrote, or the auto-assigned "opp-i0" / "opp-i1" / ...).',
    'candidates[i].insightRefs MUST reference insight IDs (either the ones the synthesis wrote, or the auto-assigned "insight-i0" / "insight-i1" / ...).',
    'Do NOT put opportunity or insight statement text in opportunityRefs / insightRefs — only IDs.',
    'If you assign your own IDs to candidates (e.g. "concept-c0"), make sure any cross-references from the downstream Direction stage can resolve to them. The simplest pattern is to omit the id and let the parser assign "concept-ma-N".',
    '',
    '# OUTPUT JSON SCHEMA',
