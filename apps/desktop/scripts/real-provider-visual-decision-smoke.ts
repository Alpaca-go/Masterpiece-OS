...
async function main(): Promise<void> {
  ...
  const compilation = await vnext.compile({
    ...
    task: {
      ...
      aspectRatio: '16:9',
      currentInstruction: [
        '生成九州美学高端医美全链生态平台的旗舰入口接待空间。',
        '必须是单一、完整、真实可进入的商业空间，包含入口、接待前台、等候咨询区和自然动线。',
        '把原视觉中的孔雀羽毛抽象为柔性层叠曲线、半透明生物组织结构和光线穿透的渐变层次，禁止照搬羽毛图案。',
      ].join(''),
      mustInclude: [
        '完整入口与接待前台',
        '等候咨询区',
  ...
  const requiredPromptSignalGroups = [
    ...
    { id: 'white-purple-system', alternatives: ['孔雀紫', '九州紫', '浅紫', '矿物紫', '珍珠白', '暖白', '米灰'] },
