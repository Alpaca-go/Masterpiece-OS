...
import type {
  EvidenceBundle,
  VNextEvidenceFileName,
  VNextEvidenceFileRecord,
...
const EVIDENCE_FILE_NAMES: VNextEvidenceFileName[] = [
  'task-contract.json',
...
export interface VNextEvidenceScannerInput {
  projectId: string;
  taskId: string;
...
function resolveFilePath(
  ...
  input: VNextEvidenceScannerInput,
...
function detectKind(name: VNextEvidenceFileName, buf: Buffer | null): VNextEvidenceFileRecord['kind'] {
...
async function readOne(
  name: VNextEvidenceFileName,
  resolvedPath: string,
): Promise<VNextEvidenceFileRecord> {
  let exists = false;
  let sizeBytes = 0;
  let sha256: string | null = null;
  let kind: VNextEvidenceFileRecord['kind'] = null;
 ...
 *     re-reading the file)
 */
export async function scanVNextEvidence(input: VNextEvidenceScannerInput): Promise<EvidenceBundle> {
  ...
  const compileNames: VNextEvidenceFileName[] = [
    'task-contract.json',
    'target-scene-projection.json',
    'prompt-source-map.json',
    'reference-trace.json',
  ...
  const files: VNextEvidenceFileRecord[] = [
    ...compileReads,
    runRec,
    outputRec,
    valRec,
 ...
 */
export async function runVNextEvidenceCheckpoint(
  input: VNextEvidenceScannerInput,
  context: VNextEvidenceValidationContext = {},
): Promise<VNextEvidenceCheckpoint> {
  const bundle = await scanVNextEvidence(input);
  // The runtime validator is a JS module; its return value
  // structurally matches VNextEvidenceCheckpoint but TypeScript
  // needs the cast.
  return validateVNextEvidenceIntegrity(bundle, context) as VNextEvidenceCheckpoint;
}
