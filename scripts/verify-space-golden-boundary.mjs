#!/usr/bin/env node
// Semantic entry point for the registered Space Golden compatibility baseline.
...
// - production-unlock.json exists (r9Unlocked=true)
...
// [4] R9 unlock
console.log('\n[4] R9 unlock...');
const unlock = parseJson(`${base}/production-unlock.json`, 'production unlock');
if (unlock) {
  ...
  fail('production-unlock.json missing/invalid');
}

// [5] Anti-regression samples
console.log('\n[5] Anti-regression samples...');
const antiIndex = path.join(root, 'space-generator/quality-baselines/current-verification/anti-regression/README.md');
