...
export function buildStrategicSynthesisPrompt(input: StrategicSynthesisPromptInput): StrategicSynthesisPromptOutput {
  ...
  const userMessage = [
    ...
    'For all four sourceMap audit-copy fields: do not omit, add, invent, transform, or summarize IDs. Runtime SOURCE TRACE IDS are authority; model-emitted values are not authority and exact set equality is re-validated by SG-12/13/14/15.',
    '',
