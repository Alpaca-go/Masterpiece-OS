...
import { validateStructureOnlyAssets } from './structure-policy.ts';
...
export function evaluateGenerationReadiness(input: {
  ...
  const structureAssetErrors = validateStructureOnlyAssets(
    input.authenticityDecisions,
    input.identityPack.structurePolicy
  );
  ...
  // §5 协议：Generation Task Definition 的阻断条件只有「直接读取 Global Ranking」
  // （由 validateStyleCarriers / taskScopedStyleCarriersReady 的 TASK_STYLE_CARRIER_NOT_SCOPED 覆盖）。
  // 文档未将「primarySubjectTypes 为空」或「outputType 缺失」列为阻断项，因此此处仅做
  // 最低限度的存在性守卫：当 GenerationTaskDefinition 存在时，必须有 outputType。
  // 注意：primarySubjectTypes 当前由 runtime 注入、无写入方，不能作为阻断依据，否则会误杀每一次生成。
  const generationTaskDefinitionReady = input.generationTaskDefinition
    ? Boolean(input.generationTaskDefinition.outputType)
    : true;
  ...
  const noUnverifiedAssetLeak = authenticityErrors.length === 0 && structureAssetErrors.length === 0;
  ...
  if (!noUnverifiedAssetLeak) blockingReasons.push(...authenticityErrors, ...structureAssetErrors);
