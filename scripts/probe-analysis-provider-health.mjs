...
//
// Usage:
//   node scripts/probe-analysis-provider-health.mjs                 # probe default (volcengine)
//   node scripts/probe-analysis-provider-health.mjs --provider qwen
//   node scripts/probe-analysis-provider-health.mjs --provider volcengine --all
//   node scripts/probe-analysis-provider-health.mjs --list          # read cached state only
//   node scripts/probe-analysis-provider-health.mjs --clear         # clear cache
//
...
function parseArgs(args) {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg === '--provider') options.provider = String(args[++i] || '').trim().toLowerCase();
      ...
      console.log(`Usage: node scripts/probe-analysis-provider-health.mjs [options]
