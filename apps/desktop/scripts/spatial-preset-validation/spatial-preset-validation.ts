...
//   validation-results/spatial-preset-validation/{brand}/{preset}/ —
...
const VALIDATION_ROOT = path.join(
  REPO_ROOT, 'space-generator', 'v1-experimental', 'validation-results', 'spatial-preset-validation', brandKey,
);
...
async function runOnePreset(
  ...
  const promptVersion = `spatial-preset-validation-${brandKey}-${preset}-1.0.0`;
  ...
  const imageRun = await imageGeneration.startCompiledCreativeTask({
    ...
    snapshot: {
      schemaVersion: '1.0',
      kind: 'spatial-preset-validation',
      brandKey,
      preset,
      spaceType,
      projectId,
...
async function main() {
  ...
  integrated.push(`- **不修改任何 production 代码**: 仅新增 \`apps/desktop/scripts/spatial-preset-validation/\` harness`);
