...
test('Qwen integration performs one request, caches it, and force-reasoning performs one new request', async () => {
  ...
  const first = await runAnalysisPipeline(input, { projectRoot, output, deepCreativeDirectorReasoner: reasoner });
  const cached = await runAnalysisPipeline(input, { projectRoot, output, deepCreativeDirectorReasoner: reasoner });
  const forced = await runAnalysisPipeline(input, {
...
test('reasoner factory is not created when the exact reasoning cache is hit', async () => {
  ...
  await runAnalysisPipeline(input, { projectRoot, deepCreativeDirectorReasoner: async () => fixtureResult });
  let factoryCalls = 0;
  const cached = await runAnalysisPipeline(input, {
    projectRoot,
