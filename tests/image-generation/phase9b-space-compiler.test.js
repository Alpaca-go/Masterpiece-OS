...
test('adaptPhase9bSource maps a ready V5 packet into Phase 9B layers', () => {
  const layers = adaptPhase9bSource({ packet: makePacket(), taskContract });
...
test('adaptPhase9bSource fails closed when no spatial signal survives', () => {
  ...
  try {
    ...
    threw = true;
    assert.ok(isSpacePhase9bInsufficient(err));
...
test('compiled prompt has Phase 9B block order with architecture before brand', () => {
  const result = compilePhase9bSpacePrompt({ packet: makePacket(), taskContract, brandKey: 'jiuzhou-aesthetics' });
...
test('compiler is deterministic', () => {
  const a = compilePhase9bSpacePrompt({ packet: makePacket(), taskContract, brandKey: 'jiuzhou-aesthetics' });
  const b = compilePhase9bSpacePrompt({ packet: makePacket(), taskContract, brandKey: 'jiuzhou-aesthetics' });
...
test('compiler does not hardcode project/brand-specific rules', () => {
  ...
  const result = compilePhase9bSpacePrompt({ packet: makePacket(), taskContract, brandKey: 'some-unknown-brand' });
