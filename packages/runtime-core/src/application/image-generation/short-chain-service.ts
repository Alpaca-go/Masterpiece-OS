...
import type { DeliverableValidatorService } from './deliverable-validator-service.ts';
import type { SimilarityAuditService } from './similarity-audit-service.ts';
import {
...
export function createShortChainGenerationService(
  projects: ProjectStore,
  projectContext: ProjectContextService,
  getImageGeneration: () => ImageGenerationService,
  getValidator?: () => DeliverableValidatorService,
  ...
  // ShortChainValidatedGenerationResult.similarityAudit. The audit is
  // ADVISORY: failures do not change flowState; they set
  // similarityAudit = 'unavailable' and block Final Acceptance.
  getSimilarityAudit?: () => SimilarityAuditService,
) {
  ...
  async function start(input: StartShortChainGenerationInput): Promise<ImageGenerationRun> {
    ...
    if (preflightBlocked || contextFingerprintStale) {
      ...
      try {
        ...
        if (recompiledTraceFile.contextFingerprint === currentContext.provenance.sourceFingerprint) {
          ...
          preflight = (recompiled.compiledPrompt as ShortChainCompiledPrompt & {
            preflightReport?: { status?: string; findings?: Array<{ code?: string }> };
          }).preflightReport;
        }
    ...
    const spaceCompilation = (compilation.compiledPrompt as unknown as {
      phase9b?: {
        compilerId?: string;
        referenceImages?: Array<{ anchorId: string; imagePath: string | null }>;
      };
    }).phase9b;
    // Reference + quality gates apply only to the production Space compiler
    // route. Compatibility fallback generations (MASTERPIECE_SPACE_COMPILER_MODE
    // unset or `vnext_legacy`) keep their previous reference semantics.
    const isProductionSpaceCompilation = isSpace && Boolean(spaceCompilation?.compilerId);
    ...
    // is the compiled prompt; the production Space branch may override
    ...
    if (isProductionSpaceCompilation) {
      ...
      if (generationBasis === 'continuation') {
        ...
        // project store has it, even when the structured visual context is stale.
        ...
        const architectureAnchorImages = (spaceCompilation?.referenceImages ?? [])
        ...
        // adapter capability no longer requires editing the Short-Chain service.
        const adapter = createSeedreamShortChainAdapter({ model: compilation.payload.model });
      ...
      const spaceCompilerBlocks = (compilation.compiledPrompt as unknown as {
        blocks?: Array<{ id: string; text?: string }>;
      }).blocks ?? [];
      const blocksById: Record<string, { id: string; text?: string }> = Object.fromEntries(
        spaceCompilerBlocks.map((b) => [b.id, b]),
      );
      const blockIds: string[] = spaceCompilerBlocks.map((b) => b.id);
      ...
      // The `short-chain-service.ts:561-568` minimum fix is preserved as the
      ...
      const gateBAdapter = createSeedreamShortChainAdapter({ model: compilation.payload.model });
  ...
  async function confirmGeneratedOutput(
    ...
    const entry: ShortChainConfirmedGeneratedOutput = {
      assetId,
      projectId,
  ...
  async function getConfirmedGeneratedOutputs(projectId: string): Promise<Record<string, ShortChainConfirmedGeneratedOutput>> {
    const session = await readSession(projectId);
  ...
  //   - ShortChainSimilarityAuditResult → audit ran and produced a result
  ...
  async function runSimilarityAuditIfTriggered(
    result: ShortChainValidatedGenerationResult,
    compilation: CompileShortChainGenerationResult,
  ): Promise<ShortChainSimilarityAuditResult | 'unavailable' | null> {
    ...
    const relation: ShortChainReferenceSceneRelation = taskContract.referenceSceneRelation ?? 'unknown';
  ...
  async function startValidated(
    ...
    const firstImage: ShortChainValidatedGenerationImageRef = {
      runId: initial.runId,
    ...
    let result: ShortChainValidatedGenerationResult;
    ...
    const finalResult: ShortChainValidatedGenerationResult = {
      ...result,
...
export type ShortChainGenerationService = ReturnType<typeof createShortChainGenerationService>;
