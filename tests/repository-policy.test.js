...
const forbiddenExtensions = new Set(['.psd', '.ai', '.cdr', '.pdf', '.zip']);
const forbiddenNames = new Set(['review.md', 'chat生图任务包.md']);
...
test('仓库不包含项目源文件或项目交付物', () => {
  ...
  for (const file of repositoryFiles()) {
    ...
    if (forbiddenNames.has(basename)) violations.push(`${file}：禁止的项目交付文件`);
